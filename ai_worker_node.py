...
# ========== 1. 基础配置 & 字体基因库 ==========
COOLMALL_API = "https://coolmall.pages.dev/api/h5/save"
...
# ========== 2. 百度实时热点采集 ==========
def safe_post(payload, headers, retries=3):
    for attempt in range(retries):
        try:
            res = requests.post(COOLMALL_API, json=payload, headers=headers, timeout=10,
...
# ========== 3. AI 商业量化重构引擎 ==========
ZHIPU_API_KEY = os.getenv("ZHIPU_API_KEY", "cd5966c9534a43d5b1a8a9ca26cdb53e.U3yC5GitxyHuLme9")
DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY", "sk-73f2969b84454361a3210f442ca796ac")
MODEL_POOL = [
...
def composite_poster_cover(img_base64_or_path, hero_title, sub_title="", english_title="", colors_dict=None,
    ...
    try:
        ...
        if not sys_font:
            sys_font = next(
                (f for f in ["C:/Windows/Fonts/msyhbd.ttc", "C:/Windows/Fonts/simhei.ttf"] if os.path.exists(f)), None)

        logging.info(f"🖋️ [字体引擎] 采用风格: {font_style} | 本地文件: {os.path.basename(sys_font) if sys_font else '默认'}")
...
# =====================================================================
# 🚀 核心修复模块：严格 rgba 格式转换器 (专治底层奇葩正则)
# 将 Python 生成的所有颜色强制转换为 "rgba(255,255,255,1)" 且绝对不带空格！
# =====================================================================
def parse_color_to_strict_rgba(c):
    ...
    if c.startswith('#'):
        ...
        if len(c) == 6: return f"rgba({int(c[0:2], 16)},{int(c[2:4], 16)},{int(c[4:6], 16)},1)"
        elif len(c) == 8: return f"rgba({int(c[0:2], 16)},{int(c[2:4], 16)},{int(c[4:6], 16)},1)"
    elif c.startswith('rgba'):
        try:
            parts = c.replace('rgba(','').replace(')','').split(',')
            # 严格去掉空格，并把 alpha 转换为整数（底层正则 \d+ 不支持小数点）
...
def make_header(conf_override=None, point_override=None, index=0):
    ...
    default_config = {
        **base_layout,
        "bgColor": parse_color_to_strict_rgba("#ffffff"),
        "color": parse_color_to_strict_rgba("#333333"),
        "logoText": "未命名",
        "fontSize": 18,
        "height": 50,
        "logo": [],
    ...
    return {
        "id": f"header_{uuid_hex()}",
        "item": {
            "type": "Header",
            "config": merge_config(default_config, conf_override),
            "category": "base",
            "editableEl": [
...
def make_image(conf_override=None, point_override=None, index=0):
    ...
    default_config = {
        **base_layout,
        "translate": [0, 0], "align": "center", "round": 0,
        "titText": "", "titFontSize": 42,
        "titColor": parse_color_to_strict_rgba("rgba(255,255,255,1)"),
    ...
    return {
        "id": f"image_{uuid_hex()}",
        "item": {
            "type": "Image",
            "config": merge_config(default_config, conf_override),
            "category": "base",
            "editableEl": [
                {"key": "titText", "name": "主标题", "type": "Text"},
...
def make_notice(conf_override=None, point_override=None, index=0):
    ...
    default_config = {
        **base_layout,
        "text": "公告内容",
        "bgColor": parse_color_to_strict_rgba("#FEF3C7"),
        "color": parse_color_to_strict_rgba("#D97706")
    }

    return {
        "id": f"notice_{uuid_hex()}",
        "item": {
            "type": "Notice",
            "config": merge_config(default_config, conf_override),
            "category": "base",
            "editableEl": [
                {"key": "text", "name": "公告内容", "type": "Text"},
...
def make_list(conf_override=None, point_override=None, index=0):
    ...
    default_config = {
        **base_layout,
        "sourceData": [], "isSearch": False, "padding": 16, "round": 8, "imgSize": "60", "fontSize": 16,
        "color": parse_color_to_strict_rgba("rgba(60,60,60,1)")
    }
...
def make_form(conf_override=None, point_override=None, index=0):
    base_layout = {"baseTop": 0, "baseLeft": 0, "baseRadius": 0, "baseRotate": 0, "baseScale": 100, "baseHeight": 100,
                   "baseWidth": 100}
    default_config = {
        **base_layout, "title": "抢占名额", "fontSize": 20,
...
def build_native_schema(blueprint):
    ...
    cover_base64 = composite_poster_cover(
        cover_base64,
        blueprint.get('hero_title', ''),
        blueprint.get('hero_subtitle', ''),
        blueprint.get('english_title', ''),
        colors,
        blueprint.get('font_style', 'tech')
    )
    cover_h = 281
        ...
        if mod == "Notice":
            ...
            for i, f in enumerate(features[:2]):
                ...
                try:
                    ...
                    icon_url = ""
                # 🖼️ Python这里输出的图片格式已经是严谨的 [{"url": "..."}] 数组，完全契合底层的图片库要求！
...
def push_to_mall(page_title, schema_json, cover_url):
    payload = {"workId": f"H5_{int(time.time())}", "title": page_title, "subTitle": "全自动营销海报", "cover_url": cover_url,
               "schema": schema_json, "category": "h5", "is_published": 1}
    success, resp = safe_post(payload, {"Content-Type": "application/json", "x-role": "admin", "x-user-id": "1"})
    if success:
        logging.info(f"✅ 【{page_title}】 上架成功！")
    else:
        logging.error(f"❌ 上架失败：{resp}")


# ========== 7. 启动流水线 ==========
if __name__ == "__main__":
    print("🚀 酷猫爆款印钞机 (终极闭环版：自动适配前端对象结构 + 取色器不罢工)")
    for tier in ["Tier_1_Top10", "Tier_2_Top100"]:
        ...
        time.sleep(1)
        print("-" * 60)
    print("🎉 任务完成！这波生成的模板右侧颜色块绝对不会变白了！")
...
// 🧬 批准 AI 的进化技能树节点：升级版（自带 Git 自动快照与失败回滚保护）
const { execSync } = require('child_process');

app.post('/api/ai/approve-skill', verifyPermission(['admin']), (req, res) => {
    const { id } = req.body;
    const propPath = path.join(__dirname, 'data', 'skill_tree_proposals.json');
    const targetRepoDir = path.join(__dirname, '../My-H5-Empire'); // 前端源码仓库路径

    if (fs.existsSync(propPath)) {
        try {
            let proposals = JSON.parse(fs.readFileSync(propPath, 'utf8'));
            let target = proposals.find(p => p.id === id);
            if (target) {
                // 🛡️ 第一步：在修改源码前，自动打一个 Git 备份存档点
                try {
                    execSync('git add . && git commit -m "backup: AI 进化前的安全存档点"', { cwd: targetRepoDir });
                } catch (e) {
                    console.log('ℹ️ 当前无未提交更改，跳过前置备份');
                }

                target.status = 'approved';
                fs.writeFileSync(propPath, JSON.stringify(proposals, null, 2), 'utf8');

                // 🚀 第二步：写入 AI 生成的新功能插件文件
                const pluginDir = path.join(targetRepoDir, 'src/plugins');
                if (!fs.existsSync(pluginDir)) {
                    fs.mkdirSync(pluginDir, { recursive: true });
                }

                const codeContent = `/**
 * 🧬 赛博天网自主进化生成模块 (已纳入 Git 保护)
 * 提案标题: ${target.title}
 * 核心描述: ${target.desc}
 * 生成时间: ${new Date().toLocaleString()}
 */
export const autoGeneratedFeature_${id} = () => {
    console.log("🚀 成功加载 AI 自主进化模块: ${target.title}");
};
`;
                const targetFilePath = path.join(pluginDir, `${id}.ts`);
                fs.writeFileSync(targetFilePath, codeContent, 'utf8');

                // 🧪 第三步：健康自检（尝试编译或检查语法，这里以 Git 状态检查模拟）
                try {
                    // 如果代码有严重冲突，这里可以捕获异常
                    execSync('git add .', { cwd: targetRepoDir });
                    res.json({ code: 200, msg: '进化代码已安全合入，并已自动创建 Git 备份！' });
                } catch (compileErr) {
                    // ❌ 发生异常，瞬间触发自动回滚！
                    execSync('git reset --hard HEAD^', { cwd: targetRepoDir });
                    return res.status(500).json({ code: 500, msg: '新代码自检未通过，已触发 Git 自动回滚，系统安全！' });
                }
                return;
            }
        } catch (e) {
            return res.status(500).json({ code: 500, msg: '进化链路中断: ' + e.message });
        }
    }
    res.status(404).json({ code: 404, msg: '找不到对应的进化提案' });
});

// ⏪ 紧急回滚：一键撤销 AI 最近一次的代码进化
app.post('/api/ai/rollback', verifyPermission(['admin']), (req, res) => {
    const targetRepoDir = path.join(__dirname, '../My-H5-Empire');
    try {
        // 强制回滚到上一个干净的 Git 节点
        execSync('git reset --hard HEAD^', { cwd: targetRepoDir });
        res.json({ code: 200, msg: '⏪ 成功回滚到上一个安全版本！' });
    } catch (e) {
        res.status(500).json({ code: 500, msg: '回滚失败: ' + e.message });
    }
});

// 📥 全网统一日志上报网关
app.post('/api/logs/collect', (req, res) => {
    const { source, level, message, details } = req.body; //来源、级别、核心信息、详情
    db.run(
        `INSERT INTO system_logs (source, level, message, details, created_at) VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)`,
        [source || 'system', level || 'info', message || '', JSON.stringify(details || {})],
        (err) => {
            if (err) return res.status(500).json({ code: 500, msg: '日志写入失败' });
            res.json({ code: 200, msg: '日志已安全归档' });
        }
    );
});
